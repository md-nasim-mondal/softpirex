...
const Register = () => {
  ...
  useEffect(() => {
    if (errors.name) {
      toast.error(errors.name.message || "Name is required!", { duration: 2000 });
    }
    if (errors.picture) {
      toast.error(errors.picture.message || "Profile picture is required!", { duration: 2000 });
    }
    if (errors.email) {
    ...
    if(imageUploadError){
        toast.error(imageUploadError || "Cannot upload image, try again later!", { duration: 2000 });
    }
  }, [errors, imageUploadError]);

  const handleRegister: SubmitHandler<RegistrationFormInputs> = async (data) => {
    const { picture } = data;
    ...
    <section className="mx-6 md:mx-10 py-2 md:py-8 p-2 md:px-4 flex flex-col items-center">
      ...
      <div className="flex-1 flex flex-col items-center gap-2 lg:w-1/2">
        ...
        >
              ...
              <div className="w-full">
                <div className="relative w-full">
                  <input
                    ...
                    onChange={(e) =>
                      setImageFileName(
                        e.target.files?.[0]?.name || "Upload Your Profile Picture"
                      )
                    }
                  />
                  <label
              ...
              <div className="relative w-full">
                <input
                  {...register("password", {
                    ...
                    validate: {
                      isCapital: (value) =>
                        /[A-Z]/.test(value) || "Password must contain uppercase!",
                      isNumeric: (value) =>
          ...
          >
            {imageUploading ? buttonLoader : "Register New Account"}
          </button>
